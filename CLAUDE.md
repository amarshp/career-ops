...
| Says "research [country] resume norms" or "set up for [country]" | `country-resume` |
...
### CV Source of Truth — Three-Layer Model

Resumes are generated by stacking three layers at generation time:

| Layer | Source | What it controls |
|-------|--------|-----------------|
| 1 — Master CV | `cv.md` | Facts, metrics, experience, projects (never modified) |
| 2 — Country norms | `config/resume-norms/{region}.md` | Photo, nationality, visa statement, length, format, work-auth wording |
| 3 — Per-job tailoring | `modes/pdf.md` | Keywords, archetype framing, project reordering, summary |

**Rules:**
- `cv.md` is the canonical CV — NEVER modify it to apply norms or keywords; apply at generation time only
- `config/resume-norms/{region}.md` must exist before any resume is generated for that region; create it via `modes/country-resume.md` if missing
- `article-digest.md` has detailed proof points (optional, used by Layer 3)
- **NEVER hardcode metrics** -- read them from `cv.md` and `article-digest.md` at evaluation time
- Pre-researched norms files: `config/resume-norms/uae.md`, `config/resume-norms/sgp.md`, `config/resume-norms/usa.md`
...
- **Default: stop before Submit for user review.** Fill forms, draft answers, generate PDFs -- then pause for the user to review and click Submit themselves.
- **Exception — autonomous mode:** If the user explicitly says "fully autonomous", "auto-submit", "submit automatically", or "do not pause for confirmation" at the start of the session, you MAY submit without pausing. The hard-skip rules (score ≥ 3.5, YOE ≤ 7, role must build AI) are your quality gate. Stop only for CAPTCHAs that CapSolver cannot solve, OTPs, or password prompts.
